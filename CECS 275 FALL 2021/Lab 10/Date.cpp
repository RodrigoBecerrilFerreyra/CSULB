...
void Date::isValid(int day_, int month_, int year_)
{
    // month validation
    ...
    if(day_ > daycap || day_ < 1)
        throw InvalidDay(day_, month_, leapYear_);

    // if function reaches this point, no errors were detected.
}
...
std::istream& operator>>(std::istream &is, Date &date)
{
    ...
    try
    {
        Date::isValid(tempday, tempmonth, tempyear);
    }
    ...
    // function does not get here if exception is thrown
